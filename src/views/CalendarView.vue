  ...
  <div class="pb-32 px-4 pt-2 max-w-md mx-auto space-y-4">
      ...
      <div>
        <h2 class="text-xl font-black flex items-center gap-2" :class="store.settings.themeMode === 'light' ? 'text-slate-900' : 'text-white'">
          ...
          <span class="text-xs px-2.5 py-0.5 rounded-full font-mono font-bold"
                :class="store.settings.themeMode === 'light' ? 'bg-amber-500/20 text-amber-800 border border-amber-500/30' : 'bg-amber-500/20 text-amber-400 border border-amber-500/40'">
            {{ store.workoutLogs.length }} 次打卡
          </span>
        </h2>
        <p class="text-xs mt-0.5" :class="store.settings.themeMode === 'light' ? 'text-slate-600 font-medium' : 'text-zinc-400'">
          历史足迹 · 做工容量 · 周期日程
        </p>
      </div>
      <button @click="jumpToToday" 
              class="px-3 py-1.5 active:scale-95 text-xs rounded-xl border transition-all cursor-pointer font-bold"
              :class="store.settings.themeMode === 'light' ? 'bg-white hover:bg-slate-100 text-slate-800 border-slate-300 shadow-xs' : 'bg-zinc-900 hover:bg-zinc-800 text-zinc-300 border-zinc-800'">
        回到今天
      </button>
    </div>

    <!-- Calendar Month Card (Apple Activity / Modern Fitness Style) -->
    <div class="rounded-3xl p-4 shadow-xl space-y-3 transition-colors"
         :class="store.settings.themeMode === 'light' ? 'bg-white border border-slate-200/90 shadow-sm text-slate-800' : 'bg-zinc-900/90 border border-zinc-800 text-zinc-100 shadow-xl'">
      ...
      <div class="flex items-center justify-between px-1">
        ...
        <div class="flex items-center gap-1.5">
          <button @click="prevMonth" 
                  ...
                  :class="store.settings.themeMode === 'light' ? 'bg-slate-100 hover:bg-slate-200 text-slate-800 border-slate-200 font-bold' : 'bg-zinc-950 hover:bg-zinc-800 text-zinc-400 hover:text-zinc-100 border-zinc-800/80'"
                  title="上一月">
            ‹
          </button>
      ...
      <!-- Calendar Days Grid (Clean, Breathing Layout - Zero Harsh Waffle Grid) -->
      <div class="grid grid-cols-7 gap-1 sm:gap-1.5">
        <div v-for="(cell, idx) in calendarCells" :key="idx"
             ...
             :class="[
               ...
               selectedDateStr === cell.dateStr 
                 ? (store.settings.themeMode === 'light' 
                     ? 'bg-slate-900 text-white shadow-md ring-2 ring-slate-900/30 font-bold' 
                     : 'bg-amber-400 text-zinc-950 shadow-lg shadow-amber-400/20 font-black') 
                 : (cell.isToday 
                     ? (store.settings.themeMode === 'light' 
                         ? 'ring-2 ring-amber-500 bg-amber-50/70 text-amber-900 font-bold' 
                         : 'ring-2 ring-amber-400/80 bg-amber-500/10 text-amber-400 font-bold') 
                     : (store.settings.themeMode === 'light' 
                ...
                :class="[
                  selectedDateStr === cell.dateStr
                    ...
                    : (cell.isToday 
                        ...
                        : (store.settings.themeMode === 'light' ? 'text-slate-800 font-medium' : 'text-zinc-400'))
                ]">
            {{ cell.day }}
          </span>

          <!-- Status badge: Only rendered for Real Workouts or Deload Shield (Zero Confetti Dots) -->
          ...
          <!-- Bottom micro indicator if volume exists -->
          <div class="h-1 flex items-center justify-center">
            <span v-if="cell.hasLog && cell.volume > 0" 
                  class="block w-1 h-1 rounded-full"
                  :class="selectedDateStr === cell.dateStr ? (store.settings.themeMode === 'light' ? 'bg-white' : 'bg-zinc-950') : 'bg-emerald-500'"></span>
          </div>
      ...
      <div class="flex items-center justify-between pt-2 border-t text-xs"
        ...
        <div class="flex items-center gap-2.5">
          ...
          <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-sky-500"></span> 拉日</span>
          <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-purple-500"></span> 腿日</span>
          <span class="flex items-center gap-1"><span class="w-2 h-2 rounded-full bg-emerald-500"></span> 休息日</span>
        </div>
        <span class="opacity-70">仅显真实打卡</span>
      </div>
    ...
    <div class="rounded-3xl p-4 shadow-xl space-y-3 transition-colors"
      ...
      <div class="flex items-center justify-between pb-2 border-b" 
           :class="store.settings.themeMode === 'light' ? 'border-slate-100' : 'border-zinc-800'">
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs font-semibold" :class="store.settings.themeMode === 'light' ? 'text-amber-800' : 'text-amber-400'">
      ...
      <div v-if="selectedDateLogs.length" class="space-y-3">
        <div v-for="log in selectedDateLogs" :key="log.id"
          ...
          <div v-else class="text-xs text-emerald-500 font-medium py-1">
            ✅ 当天标记为完全休息日
          </div>
