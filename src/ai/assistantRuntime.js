...
9. 【最高优先级指令 - 器械外观与实物照片】：当学员询问某个健身房器械“长什么样/长啥样”、怎么认出它、怎么调节座椅插销时，必须第一轮立即调用 get_gym_machine_appearance 工具！在回答中必须输出 Markdown 图片（![器械名实物照片](图片地址)）展示器械真实照片，同时附带直达网址链接（[🔗 点击打开【器械名】高清实物图](图片地址)），并详细说明其外观识别特征与插销调节要点。`;
